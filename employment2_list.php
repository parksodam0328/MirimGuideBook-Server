...
<?
	if($mode=='search') {
		if($company_name) {
			$sear_char .= " and company_name like '%$company_name%'";
		}
		if($field) {
			$sear_char .= " and field like '%$field%'";
		}
		if($website) {
			$sear_char .= " and website like '%$website%'";
		}
	}

	$query = "select * from employment2 where 1 $sear_char";
	$result = mysql_query($query, $connect);
	$total = mysql_num_rows($result);
?>

<form name="mb" method="post" action="employment2_list.php">
...
<input type="text" name="company_name" value="<?=$company_name?>" size=20 class="input">
	</td>
<td width="150" height="20" align="center" bgcolor="#F1F1F1">
		<b>분야</b>
	</td>
	<td width="250" height="20">
<input type="text" name="field" value="<?=$field?>" size=20 class="input">
	</td>
	</tr>
	<tr class="text">
	<td width="150" height="20" align="center" bgcolor="#F1F1F1">
	...
	<td width="250" height="20">
<input type="text" name="website" value="<?=$website?>" size=20 class="input">
	</td>
</tr>
<tr>
	<td colspan="4">
...
<tr>
	<td align="center" bgcolor="#f5f5f5"><strong>회사명</strong></td>
	<td align="center" bgcolor="#f5f5f5"><strong>분야</strong></td>
	<td align="center" bgcolor="#f5f5f5" colspan="2"><strong>홈페이지 주소</strong></td>
...
<?
	...
	$query4 = "select * from employment2
 			   where 1 $id
...
?>
	<tr bgcolor="#ffffff">
		<td align="center"><a href="javascript:open_win('employment2_view.php?num=<?=$rows4[id]?>','nwin','scrollbars=no, width=600, height=400')"><?=$rows4[company_name]?></a></td>
		<td align="center"><?=$rows4[field]?></a></td>
		<td align="center">
			<?=$rows4[website]?></td>
		<td align="center" colspan="2">
<a href="employment2_delete.php?m_num=<?=$rows4[id]?>&page=<?=$page?>" onclick="return confirm('삭제를 하게되면 이 취업에 관한 모든 정보가 삭제됩니다. \n 정말 삭제하시겠습니까?')">
삭제</a>
		</td>
	</tr>
<?	} 
...
<?
		$url="employment2_list.php?id=$id&company_name=$company_name&mode=$mode&field=$field&website=$website";
