...
<?
	if($mode=='search') {
		...
		if($coding) {
			$sear_char .= ' and coding like \'%'.$coding.'%\'';
		}
		if($e_result2) {
			$sear_char .= ' and result like \'%'.$e_result2.'%\'';
		}
	}

	$query = "select * from experience2 where 1 $sear_char";
...
<input type="text" name="major" value="<?=$major?>" size=20 class="input">
	...
	<td width="250" height="20">
<input type="text" name="coding" value="<?=$coding?>" size=20 class="input">
	</td>
</tr>
...
<tr class="text">
	...
	<td width="250" height="20">
<input type="text" name="e_result2" value="<?=$e_result2?>" size=20 class="input">
	</td>
<tr>
	<td colspan="4">
...
<tr>
	...
	<td align="center" bgcolor="#f5f5f5" width="300"><strong>코드</strong></td>
	<td align="center" bgcolor="#f5f5f5" width="200"><strong>결과</strong></td>
...
<?
	...
	$query3 = "select * from experience2
 			   where 1 $id
...
?>
	<tr bgcolor="#ffffff">
		...
		<td align="center">
			<a href="javascript:open_win('experience2_view.php?num=<?=$rows3[id]?>','nwin','scrollbars=no, width=600, height=400')"><?=$rows3[coding]?></a></td>
		<td align="center"><?=$rows3[result]?></td>
		<td align="center" colspan="2">
<a href="experience2_delete.php?m_num=<?=$rows3[id]?>&page=<?=$page?>" onclick="return confirm('삭제를 하게되면 이 체험의 모든 정보가 삭제됩니다. \n 정말 삭제하시겠습니까?')">
삭제</a>
		</td>
	</tr>
<?	} 
...
<?
		$url="experience2_list.php?id=$id&major=$major&mode=$mode&coding=$coding&result=$result";
