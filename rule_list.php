...
<?
	if($mode=='search') {
		...
		if($division_number) {
			$sear_char .= ' and division_number like ='.$division_number.'';
		}
		if($content) {
	...
	$query = "select * from rule where 1 $sear_char";
...
<input type="text" name="number" value="<?=$number?>" size=20 class="input">
	...
	<td width="250" height="20">
<input type="text" name="division_number" value="<?=$division_number?>" size=20 class="input">
	</td>
</tr>
...
<tr>
	<td align="center" bgcolor="#f5f5f5" width="80"><strong>번호</strong></td>
	<td align="center" bgcolor="#f5f5f5" width="130"><strong>구분번호</strong></td>
	<td align="center" bgcolor="#f5f5f5" colspan="2"><strong>내용</strong></td>
...
<?
	...
	$query3 = "select * from rule
 			   where 1 $id
			   order by number asc limit $cline, $p_scale1";
...
?>
	<tr bgcolor="#ffffff">
		...
		<td align="center">
			<a href="javascript:open_win('rule_view.php?num=<?=$rows3[id]?>','nwin','scrollbars=no, width=600, height=400')"><?=$rows3[division_number]?></a></td>
		<td align="center"><?=$rows3[content]?></td>
		<td align="center" colspan="2">
<a href="rule_delete.php?m_num=<?=$rows3[id]?>&page=<?=$page?>" onclick="return confirm('삭제를 하게되면 이 규정의 모든 정보가 삭제됩니다. \n 정말 삭제하시겠습니까?')">
삭제</a>
		</td>
	</tr>
<?	} 
...
<?
		$url="rule_list.php?id=$id&number=$number&mode=$mode&division_number=$division_number&content=$content";
