...
<?
	if($mode=='search') {
		if($date) {
			$sear_char .= ' and date like \'%'.$date.'%\'';
		}
		if($room) {
			$sear_char .= ' and room like \'%'.$room.'%\'';
		}
		if($apply) {
			$sear_char .= ' and apply like \'%'.$apply.'%\'';
		}
	}

	$query = "select * from entrance2 where 1 $sear_char";
	$result = mysql_query($query, $connect);
	$total = mysql_num_rows($result);
?>

<form name="mb" method="post" action="entrance2_list.php">
...
<input type="text" name="date" value="<?=$date?>" size=20 class="input">
	</td>
<td width="150" height="20" align="center" bgcolor="#F1F1F1">
		<b>장소</b>
	</td>
...
<input type="text" name="room" value="<?=$room?>" size=20 class="input">
	...
	<td width="250" height="20">
<input type="text" name="join" value="<?=$join?>" size=20 class="input">
	</td>
	</tr>
<tr>
	<td colspan="4">
...
<tr>
	<td align="center" bgcolor="#f5f5f5" width="160"><strong>날짜</strong></td>
	<td align="center" bgcolor="#f5f5f5" width="160"><strong>장소</strong></td>
	<td align="center" bgcolor="#f5f5f5" width="160"><strong>신청방법</strong></td>
	<td align="center" bgcolor="#f5f5f5" width="160"><strong>이미지</strong></td>
...
?>
	<tr bgcolor="#ffffff">
		<td align="center"><a href="javascript:open_win('entrance2_view.php?num=<?=$rows4[id]?>','nwin','scrollbars=no, width=600, height=400')"><?=$rows4[date]?></a></td>
		<td align="center"><?=$rows4[room]?></a></td>
		<td align="center"><?=$rows4[apply]?></a></td>
		<td align="center">
			<img src="/upload/entrance2/<?=$rows4[image_location]?>" width="100"></td>
		<td align="center" colspan="2">
<a href="entrance2_delete.php?m_num=<?=$rows4[id]?>&page=<?=$page?>" onclick="return confirm('삭제를 하게되면 이 입학에 관한 모든 정보가 삭제됩니다. \n 정말 삭제하시겠습니까?')">
삭제</a>
		</td>
	</tr>
<?	} 
...
<?
		$url="entrace2_list.php?id=$id&date=$date&mode=$mode&room=$room&apply=$apply";
